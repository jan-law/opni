	...
	for _, id := range existingTasks {
		for {
			...
			if err != nil {
				...
				ctrl.logger.Warn("failed to look up task (will retry)", logger.Err(err), "id", id)
			...
			case StatePending, StateRunning:
				if err := ctrl.LaunchTask(id, WithJsonMetadata(status.GetMetadata())); err != nil {
					ctrl.logger.Warn("failed to resume task (will retry)", logger.Err(err), "id", id)
					time.Sleep(time.Second)
					continue
				}
			case StateCompleted, StateFailed, StateCanceled:
				...
				if err := rw.Delete(ctx); err != nil {
					ctrl.logger.Warn("failed to clean task state", logger.Err(err), "id", id)
