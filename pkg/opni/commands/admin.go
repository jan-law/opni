			...
			if len(clusters) == 0 {
				...
				for _, c := range clusters {
					if !slices.Contains(cl, c) {
						lg.Error("invalid cluster id", "cluster_id", c)
						os.Exit(1)
					}
				}
			}
			...
			for _, cl := range clusters {
				...
				go func() {
					...
					for _, group := range rgs.Groups {
						...
						if err != nil {
							lg.Error("Failed to load rule group for cluster", "group", string(util.Must(yaml.Marshal(group))), "ClusterID", cl)
						} else {
							fmt.Printf("Successfully loaded rule group %s for clusterId %s", group.Name, cl)
						}
					}
				}()
			}
...
func BuildClusterStatsCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err != nil {
				lg.Warn("failed to query cortex stats", logger.Err(err))
			}
			clusterStats = stats
