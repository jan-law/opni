...
func BuildDebugAgentLogsStreamGetCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			streamRequest := &managementv1.StreamAgentLogsRequest{
				...
				Request: &controlv1.LogStreamRequest{
					Filters: &controlv1.LogStreamFilters{
						NamePattern: names,
						Level:       int32(logger.ParseLevel(level)),
					},
				},
			}
			...
			client, err := mgmtClient.StreamAgentLogs(cmd.Context(), streamRequest)
			if err != nil {
				return err
			}
			for {
				...
				fmt.Println(msg.Time.AsTime().Format(logger.TimeFormat), msg.Level, msg.Name, msg.Source, msg.Message, attrs.String())
			}
		},
	}

	cmd.Flags().StringVar(&since, "since", "2 hours ago", "Start time")
