...
func (s *AlertingNode) SyncNow(_ context.Context, req *capabilityv1.Filter) (*emptypb.Empty, error) {
	if len(req.CapabilityNames) > 0 {
		if !slices.Contains(req.CapabilityNames, s.capability) {
			s.lg.Debug("ignoring sync request due to capability filter:", "capability", s.capability)
			return &emptypb.Empty{}, nil
		}
		s.lg.Debug("received node sync request", "capability", s.capability)
...
func (s *AlertingNode) doSync(ctx context.Context) {
	s.lg.Debug("syncing node", "capability", s.capability)
	...
	case corev1.ConfigStatus_UpToDate:
		s.lg.Info("node config is up to date", "capability", s.capability)
	case corev1.ConfigStatus_NeedsUpdate:
		s.lg.Info("updating node config", "capability", s.capability)
...
func (s *AlertingNode) updateConfig(ctx context.Context, config *node.AlertingCapabilityConfig) error {
	...
	if err != nil {
		s.lg.Error("failed to fetch %s node id", "capability", s.capability, logger.Err(err))
		return err
	}
	s.idMu.RUnlock()
	...
	if err := eg.Error(); err != nil {
		...
		s.lg.Error("node configuration error", "capability", s.capability, logger.Err(err))
		return err
	}
...
func (s *AlertingNode) sendHealthUpdate() {
	...
	if !s.hasRemoteHealthClient() {
		s.lg.Warn("failed to send node health update, remote health client not set", "capability", s.capability)
		return
	}

	health, err := s.GetHealth(s.ctx, &emptypb.Empty{})
	if err != nil {
		s.lg.Warn("failed to get node health", "capability", s.capability, logger.Err(err))
		return
	}

	if _, err := s.healthListenerClient.UpdateHealth(s.ctx, health); err != nil {
		s.lg.Warn("failed to send %s node health updates", s.capability, logger.Err(err))
	} else {
