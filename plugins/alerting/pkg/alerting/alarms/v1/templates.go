...
func (p *AlarmServerComponent) fetchKubeStateInfo(ctx context.Context) (*alertingv1.ListAlertTypeDetails, error) {
	...
	if err != nil {
		lg.Error("failed to get cortex client ", logger.Err(err))
	}
	...
	for _, cl := range clusters.Items {
		...
		go func() {
			...
			if clusterHasKubeStateMetrics(ctx, adminClient, cl) {
				...
				if err != nil {
					lg.Warn("failed to extract raw series for cluster and match expr : ", "ID", cl.Id, "expr", naming.KubeObjMetricNameMatcher, logger.Err(err))
					return
				}
				result := gjson.Get(string(rawKubeStateSeries.Data), "data.result")
				if !result.Exists() || len(result.Array()) == 0 {
					lg.Warn("no result kube state metrics found for cluster : ", "ID", cl.Id, "result", result)
					return
				}
				for _, series := range result.Array() {
					...
					if !seriesInfo.Exists() || !seriesTsAndValue.Exists() {
						lg.Warn("series info or value does not exist ", "series", series)
						continue
					}
					...
					if !objName.Exists() {
						lg.Warn("failed to find object name for series ", "series", seriesInfo)
						continue
					}
					lock.Lock()
...
func (p *AlarmServerComponent) fetchCPUSaturationInfo(ctx context.Context) (*alertingv1.ListAlertTypeDetails, error) {
	...
	for _, cl := range clusters.Items {
		...
		results <- lo.Async(func() *clusterCpuSaturation {
			...
			if err != nil {
				lg.Warn("failed to extract raw series for cluster and match expr : ", "ID", cl.Id, "expr", metrics.CPUMatcherName, logger.Err(err))
				return &clusterCpuSaturation{
					clusterId: cl.Id,
				}
			}
			...
			for _, series := range result.Array() {
				...
				nodeName := ""
				if !node.Exists() {
					lg.Warn("failed to find node name for series ", "series", seriesInfo)
				...
				if !core.Exists() {
					lg.Warn("failed to find core name for series ", "series", seriesInfo)
					continue
				}
				if _, ok := cpuNodeGroup.Nodes[nodeName]; !ok {
					cpuNodeGroup.Nodes[nodeName] = &alertingv1.CpuInfo{
				...
				if !cpuState.Exists() {
					lg.Warn("failed to find cpu state for series ", "series", seriesInfo)
					continue
				}
...
func (p *AlarmServerComponent) fetchMemorySaturationInfo(ctx context.Context) (*alertingv1.ListAlertTypeDetails, error) {
	...
	for _, cl := range clusters.Items {
		...
		results <- lo.Async(func() *clusterMemorySaturation {
			...
			if !result.Exists() || len(result.Array()) == 0 {
				lg.Warn("no node memory metrics found for cluster : ", "ID", cl.Id, "result", result)
				return &clusterMemorySaturation{
					id: cl.Id,
				}
			}
...
func (p *AlarmServerComponent) fetchFsSaturationInfo(ctx context.Context) (*alertingv1.ListAlertTypeDetails, error) {
	...
	for _, cl := range clusters.Items {
		...
		results <- lo.Async(func() *clusterFilesystemSaturation {
			...
			if !result.Exists() || len(result.Array()) == 0 {
				lg.Warn("no node fs metrics found for cluster : ", "ID", cl.Id, "result", result)
				return &clusterFilesystemSaturation{
					clusterId: cl.Id,
				}
			}
